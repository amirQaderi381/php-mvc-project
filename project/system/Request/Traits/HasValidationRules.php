...
trait HasValidationRules
{
    ...
    protected function maxStr($name, $count)
    {
        if ($this->checkFieldExist($name)) {
            if (strlen($this->request[$name]) >= $count && $this->checkFirstError()) {
                $this->setError($name, "$name max length equal or lower than $count charcater");
            }
        }
    }
    ...
    protected function minStr($name, $count)
    {
        if ($this->checkFieldExist($name)) {
            if (strlen($this->request[$name]) <= $count && $this->checkFirstError($name)) {
                $this->setError($name, "$name min length equal or upper than $count charcater");
            }
        }
    }
    ...
    protected function confirm($name)
    {
        if($this->checkFieldExist($name))
        {
            $fieldName="comfirm_".$name;
            if(!isset($this->$fieldName))
            {
              $this->setError($name,"$fieldName is not exist");
            }
