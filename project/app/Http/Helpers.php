...
function paginateView($data, $perPage)
{
  ...
  $paginateView .= (($currentPage + 2) <= $totalPage) ? '<li><a href="'.paginateUrl($currentPage+2).'">' . ($currentPage + 2) . '</a></li>' : '';
  $paginateView .= (($currentPage + 1) <= $totalPage) ? '<li><a href="'.paginateUrl($currentPage+1).'">' . ($currentPage + 1) . '</a></li>' : '';
  $paginateView .= ($currentPage != $totalPage) ? '<li><a href="'.paginateUrl($totalPage).'">&lt;</a></li>' : '';
...
function paginateUrl($page)
{
  ...
  {
    $_GET['page']=$page;
    $getVariables=array_map(function($value,$key){return $key."=".$value; },$_GET,
    array_keys($_GET));
